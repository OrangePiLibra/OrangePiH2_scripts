...
if [ ! "${sdcard}" = "" ]; then
    ...
    if [ ! -f $TOP/external/Legacy_patch/orange/boot0_OPI.fex ]; then
    echo "Error: external/Legacy_patch/orange/boot0_OPI.fex not found."
    exit 1
    fi

    if [ ! -f $TOP/external/Legacy_patch/orange/u-boot_OPI.fex ]; then
    echo "Error: external/Legacy_patch/orange/u-boot_OPI.fex not found."
    exit 1
    fi
    ...
    dd if=$TOP/external/Legacy_patch/output/uboot/u-boot-sunxi-with-spl.bin of=${sdcard} bs=1k seek=16400 ${_ddparm} > /dev/null 2>&1
    if [ $? -ne 0 ]; then
